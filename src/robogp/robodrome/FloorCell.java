...
public class FloorCell extends BoardCell {
    ...
    public BufferedImage getBaseImage() {
        ...
        if (isLeftRotator()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("Rl"));
        } else if (isRightRotator()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("Rr"));
        } else if (isUpgrade()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("Au"));
        } else if (isRepair()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("Ar"));
        } else if (isDock()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("D" + getDock()));
        } else if (isCheckpoint()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("Ac" + getCheckpoint()));
        }
        return res;
    }
    ...
    public BufferedImage getTopImage() {
        ...
        for (int i = 0; i < lasers.length; i++) {
            if (lasers[i]) {
                res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("L", Direction.values()[i]));
            }
        }
        res = ImageUtil.superImpose(res, getWalls(walls));
        if (hasOddPusher()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("P1", getPusherWall()));
        }
        if (hasEvenPusher()) {
            res = ImageUtil.superImpose(res, TileProvider.getTileProvider().getTile("P2", getPusherWall()));
        }
        return res;
    }
